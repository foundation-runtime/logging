...
public class CABLoggingDispatcher implements Runnable {
	/**
	 ...
	 */
	public CABLoggingDispatcher(final AsyncAppender parent, final List<LoggingEvent> buffer, final Map<String,DiscardSummary> discardMap, final AppenderAttachableImpl appenders) {
	...
	private boolean udpateLayoutIfNeeded(Appender appender, LoggingEvent event) {
		...
		if (event instanceof CabLoggingEvent) {
			...
			if (marker != null) {
				...
				Map<String, Layout> map = CABLogger.markerAppendersMap.get(marker.getName());
