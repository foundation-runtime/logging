...
public class TestUsingLog4j {
	...
	public void myTest() {
		...
		Integer one = ApplicationState.getInstance().setState(FoundationLevel.INFO, "persistent data message");
		ApplicationState.getInstance().setState(FoundationLevel.WARN, "persistent data message - WARN");
		...
		for (int i = 0; i < NUMBER_OF_ITER; i++) {
			...
			if (i == 500) {
				ApplicationState.getInstance().updateState(one, FoundationLevel.INFO, "persistent data message was updated");
				ApplicationState.getInstance().updateState(one, FoundationLevel.INFO, "persistent data message was updated");
			} else if (i == 750) {
				ApplicationState.getInstance().removeState(one);
				ApplicationState.getInstance().removeState(Integer.valueOf(112234));
			}
