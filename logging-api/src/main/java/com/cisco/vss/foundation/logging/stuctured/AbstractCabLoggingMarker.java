...
public abstract class AbstractCabLoggingMarker implements CABLoggingMarker {
	...
	private static final Map<Class<? extends CABLoggingMarker>, Multiset<Field>> markerClassFields = new ConcurrentHashMap<Class<? extends CABLoggingMarker>, Multiset<Field>>();

	private static HashMap<Class<? extends CABLoggingMarker>, Class<CABLoggingMarkerFormatter>> markersMap = new HashMap<Class<? extends CABLoggingMarker>, Class<CABLoggingMarkerFormatter>>();

	public static HashMap<String, Element> markersXmlMap = new HashMap<String, Element>();

	private CABLoggingMarkerFormatter formatter;

	private static Logger LOGGER = LoggerFactory.getLogger(AbstractCabLoggingMarker.class);

	public AbstractCabLoggingMarker() {
		super();
		...
		CABLoggingMarkerFormatter newFormatter = getFormatter(getClass());
		newFormatter.setMarker(this);
		setFormatter(newFormatter);
	}

	private static CABLoggingMarkerFormatter getFormatter(Class<? extends CABLoggingMarker> markerClass) {
		try {

			generateAndUpdateFormatterInMap(markerClass);

			Class<CABLoggingMarkerFormatter> formatterClass = markersMap.get(markerClass);
			CABLoggingMarkerFormatter newFormatter = null;
	...
	private static void generateAndUpdateFormatterInMap(Class<? extends CABLoggingMarker> markerClass) {
		...
		if (markersMap.get(markerClass) == null) {
			...
			if (shouldGenereate) {
				...
				synchronized (markerClass) {
					...
					Class<CABLoggingMarkerFormatter> formatterClass = generateMarkerClass(newClassSource, markerClass.getName() + "Formatter");
	...
	public String valueOf(String userFieldName) {
		...
		if (CABLoggerConstants.TRANSACTION_NAME.toString().equals(userFieldName)) {
			value = getName();
		} else if (CABLoggerConstants.ALL_VALUES.toString().equals(userFieldName)) {
	...
	private String buildAllValues() {
		...
		for (Entry<String, Object> entry : entrySet) {
			...
			if (!CABLoggingMarker.NO_OPERATION.equals(value)) {
				if (first) {
					first = false;
				} else {
					builder.append(",");
	...
	protected void populateUserFieldMap() {
		...
		Class<? extends AbstractCabLoggingMarker> clazz = this.getClass();
		...
		if (fieldList == null) {
			...
			while (AbstractCabLoggingMarker.class.isAssignableFrom(cls)){
		...
		for (Field field : fieldList) {
			...
			try {
				...
				if (value == null && userField.suppressNull()) {
					value = CABLoggingMarker.NO_OPERATION;
				}
	...
	@Override
	public CABLoggingMarkerFormatter getFormatter() {
		return formatter;
	}

	public void setFormatter(CABLoggingMarkerFormatter formatter) {
		this.formatter = formatter;
	}

	private static Class<CABLoggingMarkerFormatter> generateMarkerClass(String sourceCode, String newClassName) {
		...
		try {
			...
			Class<CABLoggingMarkerFormatter> clazz = (Class<CABLoggingMarkerFormatter>) fileManager.getClassLoader(null).loadClass(newClassName);
	...
	private static <T extends CABLoggingMarker> void buildClassPrefix(Class<T> markerClass, StringBuilder builder) {
		builder.append("\npackage ").append(markerClass.getPackage().getName()).append(";\n\n");
		builder.append("import ").append(CABLoggingMarkerFormatter.class.getName()).append(";\n");
		builder.append("import ").append(CABLoggingMarker.class.getName()).append(";\n");
		builder.append("import ").append(CabLoggingEvent.class.getName()).append(";\n");

		builder.append("public class ").append(markerClass.getSimpleName()).append("Formatter").append(" implements CABLoggingMarkerFormatter ").append(" {\n\n");
		builder.append("private CABLoggingMarker loggingMarker;\n\n");
	...
	private static boolean buildFormat(Element markerElement, Class<? extends CABLoggingMarker> clazz, StringBuilder builder) {
	...
	private static void buildFromAppenderElement(Element appenderElement, StringBuilder builder, boolean isDefault, String appenderName) {
		...
		if (!isDefault) {
			builder.append("if (\"").append(appenderName).append("\".equals(").append("cabLoggingEvent.getAppenderName())){\n");
		}
	...
	private static boolean buildFromAnnotations(Class<? extends CABLoggingMarker> clazz, StringBuilder builder) {
	...
	public static void scanClassPathForFormattingAnnotations() {
		...
		for (Class<?> markerClass : annotated) {
			...
			// if the marker class is indeed implementing CABLoggingMarker
			// interface
			if (CABLoggingMarker.class.isAssignableFrom(markerClass)) {

				final Class<? extends CABLoggingMarker> clazz = (Class<? extends CABLoggingMarker>) markerClass;
			...
			} else {// if marker class does not implement CABLoggingMarker
					// interface, log ERROR

				// verify the LOGGER was initialized. It might not be as this
				// Method is called in a static block
				if (LOGGER == null) {
					LOGGER = LoggerFactory.getLogger(AbstractCabLoggingMarker.class);
				}
				LOGGER.error("Formatter annotations should only appear on CAbLoggingMarker implementations");
			}
		}
